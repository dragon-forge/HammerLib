...
public final class Request
{
	...
	/**
	 * Returns the tag attached with {@code Object.class} as a key, or null if
	 * no tag is attached with that key.
	 *
	 * <p>
	 * Prior to OkHttp 3.11, this method never returned null if no tag was
	 * attached. Instead it returned either this request, or the request upon
	...
	/**
	 * Returns the tag attached with {@code type} as a key, or null if no tag is
	 * attached with that key.
	 */
	...
	public static class Builder
	{
		...
		/**
		 * A mutable map of tags, or an immutable empty map if we don't have
		 * any.
		 */
		...
		/**
		 * Attaches {@code tag} to the request using {@code Object.class} as a
		 * key.
		 */
		...
		/**
		 * Attaches {@code tag} to the request using {@code type} as a key. Tags
		 * can be read from a request using {@link Request#tag}. Use null to
		 * remove any existing tag assigned for {@code
		 * type}.
		 *
		 * <p>
		 * Use this API to attach timing, debugging, or other application data
		 * to a request so that you may read it in interceptors, event
