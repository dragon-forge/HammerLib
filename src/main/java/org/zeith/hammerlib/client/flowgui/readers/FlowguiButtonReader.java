		...
		extends GuiReader<GuiButtonObject>
{
	...
	protected GuiButtonObject readObject(KeyMap map, String name, IDataNode node)
	{
		...
		if(keys.contains(KEY_PRESS_SOUND))
		{
			...
			if(isConstant(fac))
				builder.pressSound(Cast.constant(Holder.direct(SoundEvent.createVariableRangeEvent(Resources.location(fac.get())))));
			else
				builder.pressSound(Suppliers2.map(fac, s -> Holder.direct(SoundEvent.createVariableRangeEvent(Resources.location(s)))));
		}
