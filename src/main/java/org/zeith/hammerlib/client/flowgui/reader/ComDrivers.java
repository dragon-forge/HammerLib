...
public class ComDrivers
{
	...
	public static Supplier<Component> readComponent(JsContext jsc, String from, FlowQuery query, IDataNode attributes, GuiObject self)
	{
		...
		if(cbq != null)
		{
			return () ->
			{
				try
				{
					...
					String st;
					return s instanceof Component com ? com : MoreObjects.firstNonNull(Component.Serializer.fromJsonLenient(st = Objects.toString(s)), Component.translatable(st));
				} catch(Exception e)
				{
		...
		try
		{
			return Cast.constant(Component.Serializer.fromJson(str));
		} catch(Exception e)
		{
		}
