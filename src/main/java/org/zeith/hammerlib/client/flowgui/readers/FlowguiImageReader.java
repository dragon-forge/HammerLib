		...
		extends GuiReader<GuiImageObject>
{
	...
	protected GuiImageObject readObject(KeyMap map, String name, IDataNode node)
	{
		...
		var texture = Suppliers2.map(readString(ctx, KEY_TEXTURE), Resources::locationOrNull);
		...
		driveString(ctx, KEY_SHADER, "minecraft:gui", false, shader -> image.shader = FlowguiShaderRegistry.get(Resources.locationOrNull(shader)));
