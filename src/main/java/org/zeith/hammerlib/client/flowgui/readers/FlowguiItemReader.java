	...
	protected GuiItemObject readObject(KeyMap map, String name, IDataNode node)
	{
		...
		Supplier<ItemStack> primaryFactory = () ->
		{
			...
			if(tag != null && !tag.isBlank())
			{
				try
				{
					NBTSerializationHelper.readDataPatchFromNbt(TagParser.parseTag(tag), stack);
				} catch(CommandSyntaxException e)
				{
					stack.set(DataComponents.CUSTOM_NAME, Component.literal(e.toString()).setStyle(Style.EMPTY.withColor(ChatFormatting.RED)));
				}
			}
			return stack;
		};
