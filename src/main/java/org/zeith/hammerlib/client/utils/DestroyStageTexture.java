...
{
	private static final ResourceLocation[] DESTROY_STAGES_SPRITED = new ResourceLocation[]{
	...
	public static TextureAtlasSprite getAsSprite(float progress)
	{
		if(progress == 0)
			return null;
		return RenderUtils.getMainSprite(DESTROY_STAGES_SPRITED[Math.round(progress * (DESTROY_STAGES_SPRITED.length - 1))]);
	}
}