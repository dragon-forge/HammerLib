...
public class VariableShaderProgram
{
	...
	private static final Once initShaders = Once.run(() -> MinecraftForge.EVENT_BUS.post(new InitializeShadersEvent()));

	@SubscribeEvent
	public static void reloadShaders(ResourceManagerReloadEvent e)
	{
		initShaders.call();
		if(e.isType(VanillaResourceType.SHADERS)) Minecraft.getInstance().execute(() ->
		{
			...
			implements Consumer<VariableShaderProgram>
	{
		...
		public void accept(VariableShaderProgram program)
		{
			if(program.hasCompilationFailed())
			{
				...
				ShaderErrorToast toast = new ShaderErrorToast(new StringTextComponent(errors + " Shader Error" + (errors > 1 ? "s" : "")), new StringTextComponent(program.getId() + " failed. :<"));
			...
			implements IToast
	{
		...
		@Override
		public Visibility render(MatrixStack m, ToastGui toastGui, long delta)
		{
			if(this.newDisplay)
			{
			...
			toastGui.blit(m, 0, 0, 0, 64, 160, 32);

			if(this.subtitle == null)
			{
				toastGui.getMinecraft().font.draw(m, this.title, 18, 12, -256);
			} else
			{
				toastGui.getMinecraft().font.draw(m, this.title, 18, 7, -256);
				toastGui.getMinecraft().font.draw(m, this.subtitle, 18, 18, -1);
			}
