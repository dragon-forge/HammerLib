		...
		extends ConfigElement<ConfigArray<E>>
{
	...
	{
		E e = elementToken.create(onChanged, "[" + elements.size() + "]");
	...
	{
		return elements.stream().map(ConfigElement::getValue).collect(Collectors.toList());
	}
	...
	public boolean read(BufferedReader reader, int depth, String readerStack) throws IOException
	{
		...
		while(true)
		{
			...
			E data = elementToken.create(onChanged, "[" + elements.size() + "]");
	...
	public void fromBuffer(IByteBuf buf)
	{
		...
		{
			ConfigElement<?> inst = type.create(onChanged, "[" + i + "]");
			inst.fromBuffer(buf);
