		...
		extends ConfigElement<T>
{
	...
	public boolean read(BufferedReader reader, int depth, String readerStack) throws IOException
	{
		...
		while(true)
		{
			...
			Optional<String> comm = readComment(reader, depth);
			String tokenId = StringReader.readUntilWhitespace(reader);
			ConfigToken<?> token = ConfiguredLib.getByPrefix(tokenId);
			
			if(token != null)
			{
				ConfigElement<?> data = token.create(onChanged, name);
				String name = StringReader.readName(reader, data.nameTerminator.or(r -> Character.isWhitespace((char) r) || r == '"'));
				data.name = name;
	...
	{
		int added = 0;
		for(String name : elements.keySet().stream().sorted().collect(Collectors.toList()))
		{
			ConfigElement<?> val = elements.get(name);
	...
	public void toBuffer(IByteBuf buf)
	{
		...
		for(Map.Entry<String, ConfigElement<?>> e : elements.entrySet())
		{
	...
	public void fromBuffer(IByteBuf buf)
	{
		...
		for(int i = 0; i < size; i++)
		{
			...
			ConfigElement<?> e = ConfiguredLib.getByPrefix(token).create(onChanged, name);
			e.fromBuffer(buf);
			elements.put(name, e);
		}
	}
