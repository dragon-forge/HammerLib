		...
		implements ITokenMapper<ConfigDecimal, N>
{
	...
	public void defaultValue(ConfigDecimal element, IField<?> ownerField, N defaultValue)
	{
		Optional.ofNullable(ownerField)
				.map(f ->
						f.annotation(RangeFloat.class).map(DecimalValueRange::fromFloatRange)
								.orElseGet(() -> f.annotation(RangeDouble.class).map(DecimalValueRange::fromDoubleRange).orElse(null))
				)
				.ifPresent(element::withRange);
		
		if(defaultValue != null)
		{
			if(defaultValue instanceof BigDecimal) element.withDefault((BigDecimal) defaultValue);
