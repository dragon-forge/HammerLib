		...
		implements ICompoundSerializable
{
	...
	public SerializableMethodHandle(HolderLookup.Provider lookup, CompoundTag nbt)
	{
	...
	public CompoundTag serializeNBT(HolderLookup.Provider lookup)
	{
		...
		if(instance != null)
			NBTSerializationHelper.serializeField(lookup, instance.getClass(), instance, nbt, "Instance");
		
		nbt.putInt("Args", args.length);
		for(int i = 0; i < args.length; i++)
		{
			Object arg = args[i];
			NBTSerializationHelper.serializeField(lookup, arg.getClass(), arg, nbt, "Arg" + i);
		}
	...
	public void deserializeNBT(HolderLookup.Provider lookup, CompoundTag nbt)
	{
		...
		if(nbt.contains("Instance", Tag.TAG_COMPOUND))
			this.instance = NBTSerializationHelper.deserializeField(lookup, cls, nbt, "Instance");
		this.args = new Object[nbt.getInt("Args")];
		var pars = method.getParameterTypes();
		int c = Math.min(this.args.length, pars.length);
		for(int i = 0; i < c; i++)
			this.args[i] = NBTSerializationHelper.deserializeField(lookup, pars[i], nbt, "Arg" + i);
