		...
		extends RunnableLevelAction
{
	...
	{
		this(type, new SerializableMethodHandle(level.registryAccess(), tag.getCompound("Target")));
	}
	...
	public void run(ServerLevel level)
	{
		...
		{
			HammerLib.LOG.warn("Completely ignored non-exposed call method handle: {}", handle.serializeNBT(level.registryAccess()));
			return;
		}
	...
	public CompoundTag write(Level level)
	{
		...
		if(handle != null && handle.isResolved())
			nbt.put("Target", handle.serializeNBT(level.registryAccess()));
		return nbt;
	}
