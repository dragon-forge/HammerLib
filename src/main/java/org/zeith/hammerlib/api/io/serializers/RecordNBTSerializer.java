		...
		implements INBTSerializer<T>
{
	...
	public void serialize(HolderLookup.Provider provider, String key, @NotNull T value, CompoundTag nbt)
	{
		...
		for(int i = 0; i < components.length; i++)
		{
			...
			NBTSerializationHelper.serializeField(provider, component.getType(), suppliers[i].apply(value), inner, componentNames[i]);
		}
		nbt.put(key, inner);
	}
	
	@Override
	public @Nullable T deserialize(HolderLookup.Provider provider, String key, CompoundTag nbt)
	{
		...
		for(int i = 0; i < coms.length; i++)
		{
			...
			coms[i] = NBTSerializationHelper.deserializeField(provider, com.getType(), inner, componentNames[i]);
		}
