...
final class RealBufferedSource implements BufferedSource
{
	...
	public void readFully(byte[] sink) throws IOException
	{
		...
		{
			// The underlying source is exhausted. Copy the bytes we got before
			// rethrowing.
			int offset = 0;
			while(buffer.size > 0)
			{
	...
	public String readUtf8LineStrict(long limit) throws IOException
	{
		...
		if(newline != -1)
			return buffer.readUtf8Line(newline);
		if(scanLength < Long.MAX_VALUE && request(scanLength) && buffer.getByte(scanLength - 1) == '\r' && request(scanLength + 1) && buffer.getByte(scanLength) == '\n')
		{
			return buffer.readUtf8Line(scanLength); // The line was 'limit'
			                                        // UTF-8 bytes followed by
			                                        // \r\n.
		}
	...
	public long indexOf(byte b, long fromIndex, long toIndex) throws IOException
	{
		...
		while(fromIndex < toIndex)
		{
			...
			// The byte wasn't in the buffer. Give up if we've already reached
			// our target size or if the
