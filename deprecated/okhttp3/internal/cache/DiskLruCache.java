...
import static com.zeitheron.hammercore.lib.okhttp3.internal.platform.Platform.WARN;
 ...
 * Callers should handle other problems by catching {@code
...
public final class DiskLruCache implements Closeable, Flushable
{
	...
	public synchronized void initialize() throws IOException
	{
		...
		if(fileSystem.exists(journalFile))
		{
			...
			// The cache is corrupted, attempt to delete the contents of the
			// directory. This can throw and
			// we'll let that propagate out as it likely means there is a severe
			// filesystem problem.
			try
			{
				delete();
	...
	/**
	 * Create a cache which will reside in {@code directory}. This cache is
	 * lazily initialized on first access and will be created if it does not
	 * exist.
	 *
	 * @param directory
	...
	synchronized Editor edit(String key, long expectedSequenceNumber) throws IOException
	{
		...
		{
			// The OS has become our enemy! If the trim job failed, it means we
			// are storing more data than
			// requested by the user. Do not allow edits so we do not go over
			// that limit any further. If
			// the journal rebuild failed, the journal writer will not be
			// active, meaning we will not be
			// able to record the edit, causing file leaks. In both cases, we
			// want to retry the clean up
	...
	boolean removeEntry(Entry entry) throws IOException
	{
		if(entry.currentEditor != null)
		{
			entry.currentEditor.detach(); // Prevent the edit from completing
			                              // normally.
		}
	...
	/**
	 * Returns an iterator over the cache's current entries. This iterator
	 * doesn't throw {@code
	 * ConcurrentModificationException}, but if new entries are added while
	 * iterating, those new entries will not be returned by the iterator. If
	 * existing entries are removed during iteration, they will be absent
	 * (unless they were already returned).
	 *
	 * <p>
	 * If there are I/O problems during iteration, this iterator fails silently.
	 * For example, if the hosting filesystem becomes unreachable, the iterator
	 * will omit elements rather than throwing exceptions.
	 *
	 * <p>
	 * <strong>The caller must {@link Snapshot#close close}</strong> each
	 * snapshot returned by {@link Iterator#next}. Failing to do so leaks open
	 * files!
	 *
	 * <p>
	...
	public synchronized Iterator<Snapshot> snapshots() throws IOException
	{
		...
		{
			/**
			 * Iterate a copy of the entries to defend against concurrent
			 * modification errors.
			 */
			final Iterator<Entry> delegate = new ArrayList<>(lruEntries.values()).iterator();
			
			/**
			 * The snapshot to return from {@link #next}. Null if we haven't
			 * computed that yet.
			 */
			Snapshot nextSnapshot;
			
			/**
			 * The snapshot to remove with {@link #remove}. Null if removal is
			 * illegal.
			 */
			...
			public void remove()
			{
				...
				{
					// Nothing useful to do here. We failed to remove from the
					// cache. Most likely that's
					// because we couldn't update the journal, but the cached
					// entry will still be gone.
				} finally
				{
					removeSnapshot = null;
				}
	...
	public final class Editor
	{
		...
		/**
		 * Prevents this editor from completing normally. This is necessary
		 * either when the edit causes an I/O error, or if the target entry is
		 * evicted while this editor is active. In either case we delete the
		 * editor's created files and prevent new files from being created. Note
		 * that once an editor has been detached it is possible for another
		 * editor to edit the entry.
		 */
		void detach()
		{
			if(entry.currentEditor == this)
			{
				for(int i = 0; i < valueCount; i++)
				{
					...
					{
						// This file is potentially leaked. Not much we can do
						// about that.
					}
				}
				entry.currentEditor = null;
	...
	private final class Entry
	{
		 ...
		 * guarantee that we see a single published snapshot. If we opened
		 * streams lazily then the streams could come from different edits.
		 */
		Snapshot snapshot()
		{
			...
			long[] lengths = this.lengths.clone(); // Defensive copy since these
			                                       // can be zeroed out.
			try
			{
				for(int i = 0; i < valueCount; i++)
			...
			} catch(FileNotFoundException e)
			{
				...
				// Since the entry is no longer valid, remove it so the metadata
				// is accurate (i.e. the cache
				// size.)
				try
				{
					removeEntry(this);
