...
import com.zeitheron.hammercore.lib.okio.AsyncTimeout;
...
public final class Http2Stream
{
	 ...
	 * {@link FramingSource#read}), but not yet acknowledged by sending a
	 * {@code WINDOW_UPDATE} frame on this stream.
	 */
	// Visible for testing
	...
	/**
	 * Received headers yet to be {@linkplain #takeHeaders taken}, or
	...
	/**
	 * Removes and returns the stream's received response headers, blocking if
	 * necessary until headers have been received. If the returned list contains
	 * multiple blocks of headers the blocks will be delimited by 'null'.
	 */
	...
	public void writeHeaders(List<Header> responseHeaders, boolean out) throws IOException
	{
		...
		// Only DATA frames are subject to flow-control. Transmit the HEADER
		// frame if the connection
	 ...
	 *             if this stream was initiated by the peer and a
	 *             {@link #writeHeaders} has not yet been sent.
	 */
	public Sink getSink()
	{
		synchronized(this)
	...
	/**
	 * Abnormally terminate this stream. This blocks until the
	 * {@code RST_STREAM} frame has been transmitted.
	 */
	 ...
	 * {@link #takeHeaders}, or {@link FramingSource#read} them.
	 */
	...
	{
		/**
		 * Buffer to receive data from the network into. Only accessed by the
		 * reader thread.
		 */
		...
		/**
		 * Maximum number of bytes to buffer before reporting a flow control
		 * error.
		 */
			...
			while(true)
			{
				...
				synchronized(Http2Stream.this)
				{
					...
					try
					{
						...
						{
							// Prepare to read bytes. Start by moving them to
							// the caller's buffer.
							readBytesDelivered = readBuffer.read(sink, Math.min(byteCount, readBuffer.size()));
							unacknowledgedBytesRead += readBytesDelivered;
							
							if(errorCodeToDeliver == null && unacknowledgedBytesRead >= connection.okHttpSettings.getInitialWindowSize() / 2)
							{
								// Flow control: notify the peer that we're
								// ready for more data! Only send a
						...
						{
							// Nothing to do. Wait until that changes then try
							// again.
							waitForIo();
							continue;
						}
				...
				{
					// Update connection.unacknowledgedBytesRead outside the
					// synchronized block.
				...
				{
					// We defer throwing the exception until now so that we can
					// refill the connection
					// flow-control window. This is necessary because we don't
					// transmit window updates until
					// the application reads the data. If we throw this prior to
					// updating the connection
					// flow-control window, we risk having it go to 0 preventing
					// the server from sending data.
		...
		void receive(BufferedSource in, long byteCount) throws IOException
		{
			...
			while(byteCount > 0)
			{
				...
				// If the peer sends more data than we can handle, discard it
				...
				// Discard data received after the stream is finished. It's
				// probably a benign race.
				if(finished)
				{
					in.skip(byteCount);
					return;
				...
				// Move the received data to the read buffer to the reader can
				// read it.
				synchronized(Http2Stream.this)
				{
	...
	final class FramingSink implements Sink
	{
		...
		/**
		 * Buffer of outgoing data. This batches writes of small writes into
		 * this sink as larges frames written to the outgoing connection.
		 * Batching saves the (small) framing overhead.
		 */
		...
		/**
		 * Emit a single data frame to the connection. The frame's size be
		 * limited by this stream's write window. This method will block until
		 * the write window is nonempty.
		 */
		private void emitFrame(boolean outFinished) throws IOException
		{
			...
			synchronized(Http2Stream.this)
			{
				...
				checkOutNotClosed(); // Kick out if the stream was reset or
		...
		public void close() throws IOException
		{
			...
			if(!sink.finished)
			{
				// Emit the remaining data, setting the END_STREAM flag on the
				// last frame.
				...
				{
					// Send an empty frame just so we can set the END_STREAM
					// flag.
